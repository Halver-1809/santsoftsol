...
const useForm = <T extends FormValues = FormValues>(options?: {
  ...
  const fRequired = t('fRequired');
  const minField = t('minField');
  const maxField = t('maxField');
  const min = t('min');
  const max = t('max');
  const invalidField = t('invalidField');
  const eValidation = t('eValidation');
  ...
  const defaultErrors = {
    ...
    min: min,
    max: max,
    pattern: invalidField,
    validate: eValidation,
  }
